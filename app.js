...
const PROFILE_TEAMS = ["Editorial", "Diagramação"];
...
function getAllowedTeamsByPerfil(perfil = state.perfil) {
  if (perfil === "admin") return [...PROFILE_TEAMS];
  if (perfil === "editor") return ["Editorial"];
  if (perfil === "diagramador") return ["Diagramação"];
  return [...PROFILE_TEAMS];
}

function getScopedCommentsByPerfil(perfil = state.perfil) {
  const allowed = getAllowedTeamsByPerfil(perfil);
  return state.comentarios.filter((c) => allowed.includes(c.equipe));
}

function canAccessTaskByPerfil(task, perfil = state.perfil) {
  if (!task) return false;
  return getAllowedTeamsByPerfil(perfil).includes(task.equipe);
}

function getScopedItensByPerfil(perfil = state.perfil) {
  const allowed = getAllowedTeamsByPerfil(perfil);
  return state.itens.filter((item) => allowed.includes(item.equipe));
}

function normalizeTeamToExistingProfiles(team) {
  if (PROFILE_TEAMS.includes(team)) return team;
  return LEGACY_TEAM_MAP[team] || "Editorial";
}

function isTeamAllowedForCurrentPerfil(team) {
  return getAllowedTeamsByPerfil().includes(team);
}
...
function syncTeamSelectorsForCurrentPerfil() {
  if (!els.comentarioEquipe || !els.devolverEquipe) return;

  const allowedTeams = getAllowedTeamsByPerfil();
  const firstAllowed = allowedTeams[0] || "";

  els.comentarioEquipe.innerHTML = `
    <option value="">Atribuir equipe (obrigatório)</option>
    ${buildOptionsHtml(allowedTeams)}
  `;

  els.devolverEquipe.innerHTML = buildOptionsHtml(allowedTeams, firstAllowed);
}

function migrateCommentsToExistingProfileTeams() {
  state.comentarios.forEach((comment) => {
    comment.equipe = normalizeTeamToExistingProfiles(comment.equipe);
    const thread = ensureThread(comment);
    thread.forEach((entry) => {
      if (entry.equipe) {
        entry.equipe = normalizeTeamToExistingProfiles(entry.equipe);
      }
      if (entry.equipeOrigem) {
        entry.equipeOrigem = normalizeTeamToExistingProfiles(entry.equipeOrigem);
      }
      if (entry.equipeDestino) {
        entry.equipeDestino = normalizeTeamToExistingProfiles(entry.equipeDestino);
      }
    });
  });
}
...
function ensureThread(comment) {
  ...
  const equipes = [...PROFILE_TEAMS, ...Object.keys(LEGACY_TEAM_MAP)];
...
function getTaskById(taskId) {
  return state.tarefasProjeto.find((t) => t.id === taskId && canAccessTaskByPerfil(t));
}

function getTaskByDisciplinaEtapa(disciplina, etapa) {
  return state.tarefasProjeto.find((t) => t.disciplina === disciplina && t.etapa === etapa && canAccessTaskByPerfil(t));
}
...
function renderDashboardGeral() {
  ...
  const comentariosAbertos = getScopedCommentsByPerfil().filter((c) => !c.resolvido).length;
...
function getItensFiltrados() {
  ...
  return state.itens.filter((item) => {
    if (!getAllowedTeamsByPerfil().includes(item.equipe)) return false;
...
function getTarefasFiltradas() {
  ...
  return state.tarefasProjeto.filter((task) => {
    if (!canAccessTaskByPerfil(task)) return false;
...
function renderNotificacoes() {
  const equipesArea = getAllowedTeamsByPerfil();
  const equipeAreaTexto = equipesArea.length > 1 ? equipesArea.join(" e ") : (equipesArea[0] || "sua área");

  let comentarios = getScopedCommentsByPerfil();
  ...
  if (els.notificacoesSubtitulo) {
    els.notificacoesSubtitulo.textContent = `Perfil ativo: exibindo apenas comentários das áreas ${equipeAreaTexto}, segmentados por projeto.`;
  }
}

function openComentarioViaNotificacao(commentId) {
  const comment = getScopedCommentsByPerfil().find((c) => c.id === commentId);
  if (!comment) return;
...
function renderTaskDetail(taskId) {
  ...
  if (!task) {
    showSnackbar("Esta tarefa não pertence à área do perfil ativo.");
    return;
  }
...
function applyPermissions() {
  ...
  const allowedTeams = getAllowedTeamsByPerfil();
  if (state.commentTeamFilter !== "todas" && !allowedTeams.includes(state.commentTeamFilter)) {
    state.commentTeamFilter = "todas";
    if (els.filtroComentariosEquipe) {
      els.filtroComentariosEquipe.value = "todas";
    }
  }
  if (els.filtroEquipe.value && !allowedTeams.includes(els.filtroEquipe.value)) {
    els.filtroEquipe.value = "";
  }

  renderEstruturaProjeto();
  syncTeamSelectorsForCurrentPerfil();
  renderTabela();
  renderNotificacoes();
  renderComentarios();
  renderRaizVisualEditor();
...
function renderCommentOverview() {
  ...
  els.comentariosTotais.textContent = String(getScopedCommentsByPerfil().length);
  els.comentariosEmergenciais.textContent = String(emergenciais);

  const perfilLabel = {
    admin: "Gestor de Arte: visualiza comentários atribuídos às áreas Editorial e Diagramação.",
...
function openCommentDetail(commentId) {
  ...
  renderDetailThread(comment);
  els.detailThreadInput.value = "";
  if (isTeamAllowedForCurrentPerfil(comment.equipe)) {
...
function returnActiveCommentToArea() {
  ...
  if (!isTeamAllowedForCurrentPerfil(novaEquipe)) {
    showSnackbar("Seu perfil não pode devolver comentário para essa equipe.");
    return;
  }
...
function saveComentario(forceEmergencial = false) {
  ...
  if (!isTeamAllowedForCurrentPerfil(equipe)) {
    showSnackbar("Seu perfil não pode atribuir comentário para essa equipe.");
    return false;
  }
...
function init() {
  ...
  migrateCommentsToExistingProfileTeams();
...
Nada aqui é dado de produção, mas tudo funciona como funcionaria no sistema final. Explore à vontade! 🔭`,
    ...
    if (/(bloqueio|bloqueado|travado|impedimento)/.test(t)) {
      const bloqueados = getScopedItensByPerfil().filter((i) => i.status === "Bloqueado");
    ...
    if (/(atrasad|prazo|deadline|vencid|urgente|emergencial)/.test(t)) {
      ...
      const atrasados = getScopedItensByPerfil().filter((i) => {
    ...
    if (/(comentar|pendente|aberto|thread|feedback)/.test(t)) {
      const abertos = getScopedCommentsByPerfil().filter((c) => !c.resolvido);
      if (abertos.length === 0) {
...
<li>✅ <strong>Aprovação</strong> — gestor libera a parte para produção</li>
    ...
    if (/(gargalo|gargalos|problema|critico|crítico|risco)/.test(t)) {
      const bloqueados = getScopedItensByPerfil().filter((i) => i.status === "Bloqueado").length;
      const urgentes = getScopedItensByPerfil().filter((i) => i.prioridade === "Emergencial").length;
      const abertos = getScopedCommentsByPerfil().filter((c) => !c.resolvido).length;
      return {
